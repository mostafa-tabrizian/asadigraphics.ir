...
const DesignCards = memo(({ design }: { design: IDesign }) => {
   return (
      <Link
         id={design.slug}
         key={design._id}
         href={'/design/' + hyphen(design.slug)}
         onClick={() =>
            toast(
      ...
      >
         <div className='p-1 flex flex-col justify-between shadow-slate-200 bg-white h-full shadow-lg space-y-5 rounded-xl'>
            <div className='flex justify-center mx-auto w-full relative aspect-square'>
               {design.thumbnail ? (
                  <Image
                     ...
                     src={`https://tabrizian.storage.iran.liara.space/asadi_designs/designs/${design.thumbnail}`}
                     alt={design.name}
                     layout='fill'
            ...
            <div className='mx-1 space-y-2 relative text-right'>
               <h2 className='text-xs yekan1 md:text-sm'>{design.name}</h2>
...
DesignCards.displayName = 'DesignCards'
